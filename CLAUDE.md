...
| Page title — every in-app page | `text-lg font-semibold tracking-tight`, always through `PageHeader` — which renders it in the app header (employee and parent portals) or inline (student portal); never an `<h1>` in a page |
...
- **`PageHeader`** ([src/components/ui/page-header.tsx](src/components/ui/page-header.tsx)) — the one page heading. `title` (`text-lg`), optional `icon` / `leading` (a `BackButton iconOnly`, a badge), `actions` at the trailing edge, `tabs` (a `<TabsBar className="border-b-0">`), `sticky` to pin the body rows under the app header with the bleed offsets, and `children` for extra rows (filters, readouts, chips). No `subtitle` — descriptive copy is not a heading; put live data in a meta line instead. **In the employee and parent portals the heading lives in the app header:** the shell provides a title slot ([src/hooks/use-header-slot.ts](src/hooks/use-header-slot.ts), where the hamburger used to be) and `PageHeader` portals `leading` + `icon` + `title` into it, so the page body starts with the tabs row, flush at the content's left edge, and a page with only a title renders nothing in the body. Render exactly ONE per page, at the top level of the page's return and above any loading/error branches (pass a static fallback title while data loads) — never inside a dialog, sheet, panel or tab body; two mounted at once both show in the header. Student pages (no slot) render the heading inline as row 1, sharing it with the tabs. Student and parent pages use the `PageHeader` wrapper in [src/components/portal-layout.tsx](src/components/portal-layout.tsx), which adds the back link (`backTo={null}` for the dashboard itself) and module icon over the same component.
...
- The app is wrapped in `<ThemeProvider>` ([src/components/theme-provider.tsx](src/components/theme-provider.tsx)) in [src/main.tsx](src/main.tsx). It owns two independent axes: **mode** (`light` / `dark` / `system`, default `system`, `localStorage['nucleus-ui-theme']`) and **preset** (`default` plus every key in [src/config/themes.ts](src/config/themes.ts) — central-ui's "Everyday themes" — `localStorage['nucleus-ui-theme-preset']`).
- Read/set both via `useTheme()` (`theme`, `resolvedTheme`, `setTheme`, `preset`, `setPreset`). Never read `localStorage` or `prefers-color-scheme` directly from a component. The picker is `ThemePanel` ([src/components/settings/theme-panel.tsx](src/components/settings/theme-panel.tsx)): the student Settings → Theme tab and the employee Profile settings → Theme section. The parent portal has no presets.
- **Presets are classes on `<html>`** — `themed theme-<key>`, composing with `dark`. A preset block in `index.css` declares only anchors (`--brand-*`, `--background`, `--foreground`, `--sidebar`, and `--card` / `--popover` in dark); the shared `.themed` / `.dark.themed` blocks derive every neutral. A preset may also pin `--brand-g1..3` explicitly (so its gradient matches central's) and set `--primary-foreground` in its dark block (a lifted dark primary pairs with tinted dark ink). **Adding a theme = one ~20-line CSS block pair + one entry in [src/config/themes.ts](src/config/themes.ts)** (keep the key sets in step; the pre-paint script in `index.html` whitelists keys by shape — lowercase letters only).
- **Glass** is the one preset with element chrome (aurora, frosted header/sidebar/overlays/cards). It hooks the `portal-shell` and `app-header` classes on the student and employee shells, the employee sidebar's `sidebar-panel`, the `data-slot` overlays (dialog, sheet, dropdown) and any card-shaped `rounded-* border bg-card` container that is not `sticky` / `absolute` / `fixed`. Keep those class names when restyling a shell, and build floating panels on a `data-slot` overlay or `bg-popover`, never a raw absolute `bg-card` (those stay opaque on purpose).
...
### Back button / pinned page chrome

- **Any screen with a back button must keep it pinned** — the back control must stay visible while the body scrolls. In the employee and parent portals that is automatic: pass it as `PageHeader`'s `leading` and it renders in the app header next to the title. Never hand-roll a back control in the page body.
  - [src/components/ui/back-button.tsx](src/components/ui/back-button.tsx) — `<BackButton iconOnly label onClick />` is the one standardized back control (ghost arrow; `label` is its accessible name). Put the navigation in `onClick` (router `useNavigate` / the `navigateTo` helper) — never `window.location`. The labelled form is for inline use inside a panel or sheet only.
- Employee-portal content scrolls **inside `<main>`** (see [src/components/employee-portal-layout.tsx](src/components/employee-portal-layout.tsx)), which sits below the app header. So `position: sticky; top-0` pins flush beneath that bar — no header offset needed.
- Use the shared primitives for a pinned region, don't hand-roll:
  - `<PageHeader sticky …>` ([src/components/ui/page-header.tsx](src/components/ui/page-header.tsx)) — pins the page's tabs row, actions and any filter rows flush under the app header with the `PAGE_BLEED` offsets already applied. Reference: `InsightsHeader` in [src/components/employee/insights/bits.tsx](src/components/employee/insights/bits.tsx) and [src/pages/employee/attendance-analytics.tsx](src/pages/employee/attendance-analytics.tsx).
  - [src/components/ui/sticky-header.tsx](src/components/ui/sticky-header.tsx) — `<StickyHeader className>` for a pinned region that is *not* page chrome; it wraps with `sticky top-0 z-10 bg-background`. Supply padding, a `border-b` divider, and `space-y-*` via `className`; if it must bleed to the app header, use `PAGE_BLEED` from [src/lib/page-bleed.ts](src/lib/page-bleed.ts) rather than writing the offsets.
- The pinned bar **must be opaque** (`bg-background`) so scrolled content doesn't bleed through, and use `z-10` so it stays below popovers/sheets (`z-50`).
