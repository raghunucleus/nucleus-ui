...
/**
 * Column picker + order editor for a spreadsheet export.
 *
 * Two panes: the catalog on the left (grouped and searchable) and the ORDERED
 * selection on the right. The right pane is the point — the array it holds is
 * literally the spreadsheet's left-to-right column layout, so it is reorderable
 * by drag and by arrow buttons (arrows matter: dragging is awkward with 40
 * rows, and it's the only keyboard-reachable path).
 *
 * Surface-agnostic: the caller supplies the catalog loader, the submit closure
 * and a `storageKey`, so the drive Students tab and the eligibility-check
 * search share one dialog while their bodies and endpoints stay their own.
 */

export const MAX_EXPORT_COLUMNS = 60

/** Remember the last picked columns — repeat exports shouldn't re-pick. */
function loadRemembered(storageKey: string): string[] | null {
  try {
...
function remember(storageKey: string, columns: string[]): void {
  try {
...
export function ExportColumnsDialog({
  ...
  /** Fetches the catalog. Called once per mount, on first open. */
  loadColumns: () => Promise<ExportColumnCatalog>
  /** Starts the job. Rejections surface as a toast; the dialog stays open. */
  submit: (columns: string[], format: ExportFormat) => Promise<unknown>
  /** localStorage key the picked column order persists under. */
  storageKey: string
  /** Row count under the caller's current filters, for the footer hint. */
  total: number
  title?: string
  /** Extra footer text shown only for CSV (e.g. link columns become raw URLs). */
  csvNote?: string
}) {
  const [catalog, setCatalog] = React.useState<ExportColumnCatalog | null>(null)
  ...
  React.useEffect(() => {
    ...
    void loadColumns()
      .then((c) => {
        ...
        const remembered = loadRemembered(storageKey)?.filter((k) =>
          known.has(k),
        )
        setColumns(
          remembered?.length
            ? remembered
            : c.defaultColumns.filter((k) => known.has(k)),
        )
      })
      .catch((err: unknown) => {
        if (cancelled) return
  ...
  }, [open, catalog, loadColumns, storageKey])
  ...
  const atLimit = columns.length >= MAX_EXPORT_COLUMNS

  const toggle = (key: string) =>
    setColumns((prev) =>
      prev.includes(key)
        ? prev.filter((c) => c !== key)
        : prev.length >= MAX_EXPORT_COLUMNS
          ? prev
          : [...prev, key],
    )
  ...
  const onSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      await submit(columns, format)
      remember(storageKey, columns)
      onOpenChange(false)
      toast.success(
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl gap-3">
          ...
          <div className="grid gap-4 md:grid-cols-[1fr_20rem]">
            ...
            <div className="min-w-0">
              <div className="mb-2 flex items-baseline justify-between border-b px-1 py-1.5">
                ...
                <span className="text-[11px] text-muted-foreground">
                  {columns.length}/{MAX_EXPORT_COLUMNS}
                </span>
              </div>
        ...
        <DialogFooter className="items-center gap-3 sm:justify-between">
          ...
          >
            {submitting ? <Loader2 className="size-4 animate-spin" /> : 'Export'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
